...
function channelLookup(username){
	...
	if (data.type !== null || data.type !== undefined || data.tpye !== ""){
	...
	var cGameDescriptionSource = data.type.source;
        ...
        $('.video').html('<iframe width="600px" height="337.5px" src="https://mixer.com/embed/player/'+uUsername+'"><p>Your browser does not support iframes.</p></iframe>');
        $('.chat').html('<iframe margin-top="-7px" width="600px" height="280px" src="https://mixer.com/embed/chat/'+uUsername+'"><p>Your browser does not support iframes.</p></iframe>');    })
 
		var trimmedUsername = username.trim();
		// throw in profile links
		$('.links .direct .link').html('<a href="http://mixer.msft.online/mobile-lookup?username='+trimmedUsername+'">http://mixer.msft.online/beam/mobile-lookup?username='+trimmedUsername+'</a>');
		$('.links .clean .link').html('<a href="http://mixer.msft.online/mobile-lookup?username='+trimmedUsername+'&clean=yes">http://mixer.msft.online/beam/mobile-lookup?username='+trimmedUsername+'&clean=yes</a>');
		$('.links').show();
